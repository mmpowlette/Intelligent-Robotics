...
# "return obj_handle"
##issues with code calling function. Error returns:
# External call to simCallScriptFunction failed ('cup_path@/NAO/script'): script does not exist.
count = 0
print (count)
while count != 1000:
    ...
    count +=1 
sim.stopSimulation()
# sim.callScriptFunction(funct2,NAOhandle)
# if res==sim.sim_return_ok:
...
# client = RemoteAPIClient()  #### figure how to run specific simulation
# sim = client.getObject('sim')
# error_Code, robot =sim.simxGetObjectHandle(client, '/NAO[0]', sim.simx_opmode_oneshot_wait)
# sim.simxCallScriptFunction(client, robot,sim.sim_scripttype_mainscript (0) ,to_bowl )
# sim.startSimulation()

# clientID = sim.simxStart('127.0.0.1',num, True, True, 5000, 5)
